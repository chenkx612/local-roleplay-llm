...
## P1｜旧版主观 Judge GRPO 导致质量退化（已封存）

**位置：** `output/morgana-v2/stage3-grpo/20260809-2031/run_summary.json` 及对应本地 run。

**现象：** 训练和自动稳定性门槛完成，但人工复核仅胜 2/10、明显落后 4/10；生成稳定性、角色
一致性和对话质量均分全部低于 SFT，最终状态为 `grpo_failed`。

**阶段决策：** 旧 adapter 不进入后续训练，旧主观奖励规约只保留为负向证据。当前流程在 SFT
后增加 DPO，用人工校准的偏好对学习主观质量；后续 GRPO 仅保留可自动验证的明确规则，并使用
与 DPO 分离的新 Prompt。
