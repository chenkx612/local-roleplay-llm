...
旧版从 SFT 直接训练、使用在线主观 Judge 奖励的 Stage 3 GRPO 已执行失败并封存，不进入当前
链路。Stage 4 跳过所有失败 DPO adapter，从已通过验收的冻结 SFT adapter 开始，只学习能够
稳定、自动验证的明确规则，并使用 20 条独立、规则导向且与其他 split 去重的 Prompt。

奖励完全在本地计算，不调用在线 Judge：不可读、截断、乱码、严重复读和括号错误作为硬门控；
30～90 字的简洁回复、单次“吾辈”和按场景适度使用动作获得奖励；错误自称、动作过密和格式
泄漏受到惩罚。训练固定使用 4 个候选、1 epoch、`learning_rate=5e-7` 和 `beta=0.1`。

Dev 自动门槛要求 GRPO 相对 SFT 的规则奖励均分至少提升 0.3，且至少 6/10 条严格胜出，同时
没有生成稳定性回归。匿名人工复核继续记录胜负和三维评分，但只以新增不可读、角色崩坏或视角
错位等严重问题作为阻断项。主观角色感、自然度和情绪价值不作为在线奖励。
...
对 10 条 Eval 做匿名比较并保留具体样本。长度、自称和动作密度同时报告为 Stage 4 规则结果，
但不代替三层质量判断；emoji、创造性细节和模板化只作为解释性诊断。结论必须区分 SFT 相对
Base、失败 DPO 相对 SFT、规则型 GRPO 相对 Base 和 SFT 的变化，并注明小样本、单角色、单次
训练和偏好标注偏差的限制。
...
- [x] 生成独立规则 Prompt，冻结本地规则奖励和 Stage 4 四命令。
- [ ] 从冻结 SFT adapter 完成一次有效 GRPO。
- [ ] 完成 GRPO 相对 SFT 的规则与质量回归评估。
