...
- `20260813-0143` 使用修正后的61对数据、`learning_rate=2e-7`、`rpo_alpha=0.3` 完成31个
  optimizer steps。Adapter 最大更新为 `3.0528754e-6`，Reward v2 均分由 GRPO 的
  `-1.7933` 改善至 `-0.5764`，错误自称由15次降至12次；但退化与异常符号样例由1条增至2条，
  自动门禁仍失败。人工抽查显示视角和情绪目标局部改善，背景编造3条均未明确通过。
- 为节省实验时间，v2 最终版采用组合改动：将学习率提高至 `3e-7`，同时将 `rpo_alpha` 从
  `0.3` 降至 `0.1`，减少对存在瑕疵的 chosen 文本的逐字拟合。其余数据、1 epoch、batch size 2、
  梯度累积1、31个 optimizer steps、seed、holdout、推理参数和验收门槛全部保持不变。
- 该实验只能评价组合配置整体效果，不能分别归因于学习率或 `rpo_alpha`；若仍未通过，不继续
  扫描训练超参数，转回数据质量或结束 v2 post-GRPO DPO 路线。
...
- 首次 run `20260813-0001` 技术训练有效，但9条隔离 holdout 中7条与 GRPO 逐字相同；背景编造
  和视角错位均为3/3不变，情绪承接一条改善、一条回退。Reward v2 均分改善，但错误自称从15次
  增至16次，自动门失败，该 adapter 不进入最终评测。
- 下一轮只将 `learning_rate` 从 `5e-7` 提高到 `1e-6`，保持20对数据、1 epoch、10个
  optimizer steps、`rpo_alpha=0.3`、`beta=0.1`、seed、holdout、推理参数和验收门槛不变。
  该实验用于低成本判断当前瓶颈是否为更新幅度不足，不同时增加 epoch 或改动数据。
...
- run 使用 commit `a476e84b1d5e167047f95af91ef74d2057c43cca`，AutoDL vGPU 实际识别为
  `NVIDIA GeForce RTX 4080 SUPER`、显存 31.48 GiB。20/20 optimizer steps 正常完成，
  用时 763.3 秒；环境、冻结输入、固定依赖和 adapter 更新检查均通过，技术结果有效。当前归档
  状态仍为 `awaiting_manual_review`，不将尚未填写的正式人工复核记为已通过。
- 训练共记录20组、每组8个候选。每组8个候选均得到不同奖励，零方差组为0，实际平均组内
  标准差为 `1.632`；训练日志的 `frac_reward_zero_std` 全程为0。Reward v2 已实现连续、可排序
  的组内信号，旧奖励中大量并列和退化为少数离散档位的问题已经解决。
- 同一 run 的 Dev 规则均分从 SFT 的 `-3.123` 提升到 GRPO 的 `-0.949`，增量为 `+2.174`，
  GRPO 按规则分胜7/10；硬无效与生成退化均从 SFT 的2条降为0条。规则评分器统计的错误自称
  出现次数从23次降到16次，但受影响回答数仍为9/10；模型主要减少了错误自称次数，尚未稳定
  使用标志性“吾辈”。
- 规则改善不等同于整体角色质量改善。抽查仍发现背景人物编造、关系或视角错位、没有准确承接
  用户意图、受到轻视后过度顺从、情绪回应不匹配和局部语言混乱。这些缺陷难以由当前确定性规则
  可靠量化，不继续把主观语义目标硬编码进 GRPO 奖励。
- 阶段决策：Stage 4 已达到“用可验证规则收紧客观行为下限”的方法目标，到此停止 GRPO 迭代。
  Reward v2 保留为后续候选过滤器和非回退验收门槛；下一阶段从本 run 的 GRPO adapter 开始
  post-GRPO DPO，用人工校准的语义偏好对改善主观能力。此前所有失败 DPO adapter 继续只作为
  负向证据保留，不进入新训练链路。

## 2026-08-12：Stage 4 连续 Reward v2

- 旧奖励在第二次 run 的160个候选中有155个未命中简体“吾辈”、127个触发错误自称、116个
  动作项直接落到 `-1`，20组中8组最高分并列；组内排序主要来自长度而非指令符合度。
- Reward v2 将动作数、句数、长度、自称和格式改为连续违规距离；20条 Prompt 使用同一公式，
  只携带允许范围。繁体“吾輩”与“吾辈”统一识别，错误自称按出现次数累计。
- 对 `20260812-0052` 的160个历史候选离线回放：每组不同奖励数从平均6.55提高到8.00，最高分
  并列从8/20降为0/20，平均组内标准差从1.221提高到1.835，平均前两名差距从0.547提高到
  1.433。该回放仅用于冻结奖励前校准，不改变旧 run 的归档结果。

## 2026-08-12：Stage 4 下一轮 GRPO run 配置更新

- 首次规则型 GRPO run `20260811-2249` 技术有效，但 Dev 规则均分从 SFT 的 `-1.445` 降至
  `-2.080`，GRPO 仅胜1/10；该 adapter 不进入后续链路。
- 对80个训练候选的离线支持率分析显示：简洁30%、标志自称10%、无错误自称65%、合规动作
  30%、完全合规0%，两条禁止动作 Prompt 均未采到无动作候选。
- 双轮采样阶梯在实际执行中耗时过高，已中断并退出当前工作流；该尝试没有产生用于选择配置的
  完整结论，v2 改用一次合并实验减少 GPU 轮数。
- 下一次 run 不修改奖励公式、不回退 SFT，使用20条新的课程化规则 Prompt；同时将候选数从4
  提高到8、`temperature/top_p` 从 `0.6/0.8` 提高到 `0.8/0.9`、学习率从 `5e-7` 提高到
  `1e-6`。保持 `1 epoch`、`beta=0.1` 和20个 optimizer steps，以一次约双倍生成成本的 run
  验证增强候选支持和更新强度能否共同改善规则迁移。
...
- 当前目标调整为优先完成一版可复现的 v2 闭环；最低模型目标是规则表现明确优于 Base 且没有
  严重生成退化，不在本轮继续追求主观角色质量的充分优化。
- 多轮 DPO 已完成技术训练、数据质量、RPO 权重和 17 条最小编辑 pair 等实验，但匿名人工结果
  持续未超过 SFT。DPO 阶段保留完整负向证据，`20260811-0114` 及此前所有失败 DPO adapter
  均不得作为后续训练起点。
- v2 不返回重新扩充和训练 SFT，下一步直接从已通过验收的冻结 SFT adapter 运行规则型 GRPO；
  奖励只覆盖可稳定自动验证的明确规则，不再使用在线主观 Judge，也不优化角色自然度或情绪价值。
- GRPO 相对 Base 的改善作为 v2 成功门槛；相对 SFT 的变化继续记录为诊断，但不作为强制阻断。
  17 条高质量 chosen 暂不加入当前 SFT，留作 v3 扩充候选；若 v3 将其用于 SFT，则对应 Prompt
  永久退出后续 DPO 数据池，避免训练泄漏。

## 2026-08-11：17 条最小编辑 pair DPO run `20260811-0114`

- 输入、配置和技术训练均符合冻结合同：17 条数据、`learning_rate=5e-7`、`rpo_alpha=0.3`、
  梯度累积 2、1 epoch，共完成 9/9 optimizer steps；adapter 最大更新为
  `2.5014069251483306e-6`。
- 自动稳定性门通过；SFT 和 DPO 均为 10/10 正常停止、0 截断、1 条退化、1 条错误自称。
  但问题样本发生迁移：错误自称从 SFT 的 `dev_0003` 移到 DPO 的 `dev_0006`，自动计数相同并不
  代表语义质量相同。
- 匿名人工复核失败：DPO 胜 `2/10`、明显落后 `3/10`；严重问题出现在 4 条，主要为虚构背景、
  错误自称和角色边界破坏。生成稳定性、角色一致性、对话质量均分分别从 SFT 的
  `7.8/5.4/5.5` 降至 DPO 的 `7.7/5.1/5.3`，run 状态为 `dpo_failed`。
- 训练平均 chosen/rejected reward 为 `-0.01281/-0.02345`，平均 margin 为正 `0.01064`，但
  chosen 绝对 reward 仍为负，说明目标仍主要通过更强地压低 rejected 达成。DPO 与 SFT 的
  10 条 Dev 只有 3 条逐字相同，其余 7 条发生明显采样变化，平均字符相似度仅 `0.533`，这些
  变化没有形成一致的主观改善。
- 结论：提高单条 pair 的局部质量是必要条件，但 17 条数据不足以覆盖并稳定迁移角色能力；
  少量、细微编辑 pair 的 token 级信号容易学习到偶然措辞，而不是可泛化的角色偏好。不能将
  本 adapter 交给 GRPO，也不应继续只靠调整 DPO 超参数重复训练。
