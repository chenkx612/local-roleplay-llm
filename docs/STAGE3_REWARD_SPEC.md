...
回答非空、无乱码、无严重复读且无破坏性截断时，`Readable=1`；否则为 0。实现应复用
`src/roleplay/sft_eval.py` 的乱码和复读检测。`finish_reason=length` 或 ms-swift 传入
`is_truncated=true` 时一律视为破坏性截断，以保持判定可复现。
...
- 自认普通宠物：只影响身份、边界与事实。
- 编造人物或经历：影响身份、边界与事实；只有因此答非所问时才同时影响回应有效性。
