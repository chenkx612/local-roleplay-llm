"""ms-swift AsyncORM adapter for the Stage 4 local rule reward."""
...
from roleplay.grpo_rule_reward import RuleRewardEngine, RuleRewardError


class MorganaRuleRewardORM(AsyncORM):
    """Expose the deterministic reward through ms-swift's registry."""

    def __init__(self, args: Any = None, **kwargs: Any) -> None:
        super().__init__(args, **kwargs)
        output_dir = getattr(args, "output_dir", None)
        if not output_dir:
            raise RuleRewardError("ms-swift args.output_dir 不能为空")
        self.engine = RuleRewardEngine(
...
orms["morgana_rule_reward"] = MorganaRuleRewardORM
