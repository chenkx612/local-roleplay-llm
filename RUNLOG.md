...
**状态：有效训练已完成，按新版三目标待复核。** 首次运行没有产生有效 LoRA 参数更新；第三次
运行已完成有效更新，但尚未通过新版生成稳定性门槛，不能作为 GRPO 起点。
...
- Run ID：`20260808T094035Z-7918de52`；产物目录：`output/morgana-v1/stage2-sft/3/`。
- 使用既定 FP32 QLoRA 配置完成 3 epochs、12 个 optimizer step，耗时 676.511 秒；全部
  `grad_norm` 有限且为正，186/186 个 LoRA-B 张量均非零，adapter 可重载并生成非空回答。
- loss 从首步 `2.6791` 波动下降至末步 `1.9329`；adapter SHA-256 为
  `cb26676f1d9d087db3796401e12c7fe219bb6d2a3fc6bf8989edb9e7fba5eb3e`。
- 同后端 Base/SFT 各生成 30 条对齐 Dev。Base → SFT：`stop` 24 → 25，截断 6 → 5，旧版
  退化标记 24 → 10，错误自称 22 → 8；严格动作格式均为 0/30。
- 因旧版自动门槛要求严格格式率至少提高 20 个百分点，本次归档状态为 `behavior_failed`，人工
  复核未开始。该状态忠实保留当时口径，不事后改写历史产物。

### 2026-08-08 评估目标变更

第三次结果表明，严格动作格式、emoji 和固定口头禅频率对体验收益有限，却可能单独否决已经明显
改善的回复。项目目标因此收敛为三个逐层目标：

1. **生成稳定性**：完整、连贯、可读，无乱码、严重复读或破坏性截断。
2. **角色一致性**：核心身份、性格、关系、边界和语言风格成立。
3. **对话质量**：直接回应用户，自然、有意义并能承接后续对话。

格式一致性和事实可靠性不再作为独立目标。动作括号、emoji、自称频率、回答长度和创造性细节
继续记录为诊断信息；允许不违背核心设定和当前对话的合理创作，但不得擅自编造用户个人经历或
重大共同关系。新版自动门槛只负责生成稳定性，匿名人工复核按三个核心目标评分。

使用新版代码只读重评第三次产物：完整对齐、结束数、截断和复读均通过；唯一失败项是
`20260808:dev_0010` 的不可读字符序列，因此 `core_behavior_gate=false`。这比旧结论更聚焦：
第三次不是因为未遵循动作模板失败，而是仍存在一条真实的生成稳定性问题。正式人工复核需在
稳定性通过后进行。
...
- 生成稳定性、角色一致性、对话质量三层结果、诊断指标及逐模型样本观察。
...
- [x] 阶段一问题与关键工程决策。
- [x] 有效 SFT 的实际配置、日志、产物和 Dev 结果；首次运行已判无效，第三次尚未获准进入 GRPO。
